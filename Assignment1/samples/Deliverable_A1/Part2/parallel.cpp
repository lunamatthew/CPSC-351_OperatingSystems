...
int main() {
	...
	for (int i = 0; i < locations.size() && pid != 0; ++i) {
		pid = fork();
		++count;
	...
	if (pid == 0) { // If pid == 0 then is child
		...
		saveFileLocStr += std::to_string(count);
		saveFileLocStr += ".json";
		char urlCString[BUFF_SIZE];
		snprintf(urlCString, BUFF_SIZE, "https://api.open-meteo.com/v1/forecast?latitude=%f&longitude=%f&current_weather=True", locations[count-1].latitude, locations[count-1].longitude);
